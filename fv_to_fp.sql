...
ssp AS (
  ...
  --AND LOWER(online_profile_id) = '31741ebd-b4fa-4b53-8278-b9cd236ea409'
),

first_touch_breakdown AS(
SELECT
a.guid,
DATE_DIFF(min_subscription_start_date,min_visit_date, DAY) AS days_diff_first_touch
FROM adobe AS a
INNER JOIN ssp AS b
ON a.guid = b.online_profile_id
WHERE DATE_DIFF(min_subscription_start_date,min_visit_date, DAY) >= 0
GROUP BY ALL
ORDER BY days_diff_first_touch ASC
)

SELECT
ROUND(COUNT(DISTINCT CASE WHEN days_diff_first_touch BETWEEN 0 and 30 THEN guid END) / COUNT(DISTINCT guid),4) AS _0_to_30d,
ROUND(COUNT(DISTINCT CASE WHEN days_diff_first_touch BETWEEN 0 and 1 THEN guid END) / COUNT(DISTINCT guid),4) AS _0_to_1d,
AVG(days_diff_first_touch) AS avg_days
FROM first_touch_breakdown
