WITH adobe AS(
  ...
  CASE
  -- Self Help
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:sync%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:recalls%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:manuals%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%self help%' THEN LOWER(df0rgf_post_evar46)

  -- Service & Maintenance
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:service maintenance%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:maintenance schedule%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:service:tire finder%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%collision%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:coupons%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:service:service history%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:towing calculator%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%vehicle health%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:vhr%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:warranty%' THEN LOWER(df0rgf_post_evar46)

  -- Owner Account
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:account%' THEN LOWER(df0rgf_post_evar46)

  -- Owner Home
  WHEN df0rgf_post_evar11 = 'flmo:owner:home:search by vehicle' THEN LOWER(df0rgf_post_evar46)

  -- Account Dashboard
  WHEN df0rgf_post_evar11 = 'flmo:owner:account' THEN LOWER(df0rgf_post_evar46)

  -- Vehicle Dashboard
  WHEN df0rgf_post_evar11 IN ('flmo:owner:profile:home','flmo:owner:vehicle info') THEN LOWER(df0rgf_post_evar46)

  -- Onboarding
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:dyf%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_evar11 LIKE '%flmo:owner:how to videos%' THEN LOWER(df0rgf_post_evar46)

  -- FPR Specific
  WHEN df0rgf_post_evar11 IN ('flmo:owner:fpr:rewards','flmo:owner:fpr:overview','flmo:owner:fpr:dashboard') THEN LOWER(df0rgf_post_evar46)

  -- High Value CTAs (from prop5)
  WHEN df0rgf_post_prop5 = 'referall:flmc:amo:gux:welcome to your account manager' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_prop5 LIKE '%tire finder:vehicle:%' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_prop5 IN ('referral:flmo:find dealer:results','owner:tiles:schedule service:schedule service') THEN LOWER(df0rgf_post_evar46)
  ...
  WHEN df0rgf_post_prop5 = 'referral:gfa:home' THEN LOWER(df0rgf_post_evar46)
  WHEN df0rgf_post_prop5 LIKE '%owner:quick links:parts%' THEN LOWER(df0rgf_post_evar46)
...
SELECT
COUNT(DISTINCT key_action_guid) AS key_action_guids,
COUNT(DISTINCT c_services_click_guid) AS c_services_click_guids
FROM adobe
