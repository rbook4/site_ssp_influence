...
ssp AS (
  ...
  MIN(a.subscription_start_date) OVER(PARTITION BY LOWER(a.online_profile_id)) AS min_subscription_start_date,
...
purchase_pairs AS(
  ...
  b.product_sku,
  b.min_subscription_start_date
  FROM adobe AS a
  INNER JOIN ssp AS b
  ON a.guid = b.online_profile_id
  WHERE a.visit_date <= b.subscription_start_date
),

last_visit_to_purchase AS(
  SELECT
  guid,
  min_visit_date,
  ...
  product_sku,
  IFNULL(MAX(CASE WHEN visit_date < subscription_start_date THEN visit_date END), subscription_start_date) AS last_visit_date_before_purchase
  FROM purchase_pairs
  GROUP BY ALL
)

SELECT
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) = 0 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_0d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 15 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_15d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 30 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_30d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 45 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_45d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 60 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_60d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 75 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_75d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 90 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_90d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 120 THEN guid END) / COUNT(DISTINCT guid) AS _0_to_120d,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) >= 121 THEN guid END) / COUNT(DISTINCT guid) AS _121d_beyond,
AVG(DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY)) AS avg_days_diff,
COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) = 0 THEN guid END) / COUNT(DISTINCT CASE WHEN DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) BETWEEN 0 AND 15 THEN guid END) AS _0d_pct_of_0_to_15d,
COUNT(DISTINCT guid) AS guids
FROM last_visit_to_purchase
WHERE DATE_DIFF(subscription_start_date, last_visit_date_before_purchase, DAY) >= 0
--AND guid = '136a662b-04e0-b491-7ee6-fa207ee6fa20'
