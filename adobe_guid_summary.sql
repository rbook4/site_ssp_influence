WITH adobe AS(
  ...
  CASE
    WHEN LOWER(df0rgf_page_url) NOT LIKE '%webview=true%' THEN LOWER(df0rgf_post_evar46)
    ELSE NULL
  END AS non_webview_guid,
...
SELECT
COUNT(DISTINCT guid) AS total_guids,
COUNT(DISTINCT non_webview_guid) AS non_webview_guids
FROM adobe
